...
export function LeadDetailView({
  ...
  onApplyCustomStatus?: (leadId: string, status: string) => Promise<unknown> | unknown
  onApplyNextAction?: (leadId: string, nextAction: string, nextActionAt: string) => Promise<unknown> | unknown
  onBack: () => void
  ...
  const [nextActionAtInput, setNextActionAtInput] = useState(formatDateInput(lead.nextActionAt || lead.nextFollowUpAt || ''))
  ...
  const propertyName = getPropertyName(lead.propertyId) || 'Sin propiedad asignada'
  const nextAction = lead.nextAction || lead.notes || 'Definir siguiente accion'
  const advisorName = lead.assignedAgentName || 'Sin asesor'
  const advisorStatusLabel = formatAdvisorStatus(lead.advisorStatus || lead.status)
  const systemStatusLabel = formatSystemStatus(lead.systemStatus)
  const primaryStatusLabel = mode === 'coordinator' ? systemStatusLabel : advisorStatusLabel
  const canManageCustomStatus = mode === 'advisor' && Boolean(onApplyCustomStatus)
  const canManageNextAction = mode === 'advisor' && Boolean(onApplyNextAction)
  const visibleCustomStatuses = useMemo(() => {
    const statuses = new Set(customLeadStatuses.map((status) => status.trim()).filter(Boolean))
  ...
  const applyCustomStatus = async (status: string) => {
    ...
    try {
      ...
      console.warn('No se pudo actualizar el estado personalizado del lead:', error)
      setCustomStatusError('No se pudo actualizar el estado. Intenta de nuevo.')
    } finally {
  ...
  const applyNextAction = async () => {
    ...
    try {
      ...
      setNextActionError('No se pudo guardar la proxima accion. Intenta de nuevo.')
    } finally {
      ...
      <Pressable style={styles.detailScreen} onPress={Keyboard.dismiss}>
          ...
          <View style={styles.profileCard}>
            <View style={styles.profileTopRow}>
              <View style={styles.profileAvatar}>
                <Text style={styles.profileAvatarText}>{getInitial(lead.name)}</Text>
              </View>
                ...
                <View style={styles.statusBadgeStack}>
                  ...
                  {mode === 'coordinator' ? (
                    <View style={[styles.stageBadge, styles.advisorStageBadge]}>
                      ...
                      <Text style={[styles.stageText, styles.advisorStageText]}>{advisorStatusLabel}</Text>
                    </View>
                  ) : null}
                </View>
              </View>
            ...
            <View style={styles.infoGrid}>
              ...
              <DetailMetric icon={<UserRound size={13} color="#0c6740" />} value={`Asesor: ${advisorName}`} wide />
            ...
            <View style={styles.detailSection}>
              <View style={styles.customStatusHeader}>
                ...
                <Text style={styles.customStatusSubtitle} numberOfLines={1}>
                  {isLoadingCustomLeadStatuses ? 'Cargando estados...' : advisorStatusLabel}
                </Text>
              </View>
              ...
              {visibleCustomStatuses.length > 0 ? (
                <View style={styles.statusChipList}>
                  {visibleCustomStatuses.map((status) => {
                    ...
                    return (
                      <TouchableOpacity
                        key={status}
                        style={[styles.customStatusChip, isActive && styles.customStatusChipActive]}
                        activeOpacity={0.85}
                        disabled={isSavingCustomStatus}
                        onPress={() => applyCustomStatus(status)}
                      >
                        <Text
                          style={[styles.customStatusChipText, isActive && styles.customStatusChipTextActive]}
                          numberOfLines={1}
                        >
                ...
                <Text style={styles.customStatusEmpty}>Sin estados personalizados guardados.</Text>
              )}
              ...
              <View style={styles.customStatusForm}>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    (!customStatusInput.trim() || isSavingCustomStatus) && styles.customStatusButtonDisabled,
                  ]}
                  activeOpacity={0.85}
              ...
              {customStatusError ? <Text style={styles.customStatusError}>{customStatusError}</Text> : null}
            </View>
          ) : null}
          ...
          <View style={styles.detailSection}>
            ...
            <View style={styles.nextActionCard}>
              ...
              <View style={styles.nextActionCopy}>
                <Text style={styles.nextActionTitle} numberOfLines={1}>{nextAction}</Text>
                <Text style={styles.nextActionMeta} numberOfLines={1}>
                  {lead.nextActionAt ? formatDate(lead.nextActionAt) : lead.nextFollowUpAt ? formatDate(lead.nextFollowUpAt) : 'Sin fecha programada'}
                </Text>
              ...
              <View style={styles.nextActionForm}>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    (!nextActionInput.trim() || !nextActionAtInput.trim() || isSavingNextAction) && styles.nextActionSaveButtonDisabled,
                  ]}
                  activeOpacity={0.85}
                  disabled={!nextActionInput.trim() || !nextActionAtInput.trim() || isSavingNextAction}
                  onPress={applyNextAction}
                >
                  <Text style={styles.nextActionSaveButtonText} numberOfLines={1}>
                    {isSavingNextAction ? 'Guardando' : 'Guardar proxima accion'}
                  </Text>
                </TouchableOpacity>
                {nextActionError ? <Text style={styles.customStatusError}>{nextActionError}</Text> : null}
              </View>
            ) : null}
            <View style={styles.quickActionsGrid}>
              ...
              <QuickAction icon={<CalendarPlus size={19} color="#0c6740" />} label="Agendar cita" onPress={() => undefined} />
              <QuickAction icon={<Repeat2 size={19} color="#0c6740" />} label="Cambiar etapa" onPress={() => undefined} />
            </View>
          </View>
        ...
        <View style={styles.detailBottomDock}>
          <View style={styles.detailBottomActions}>
            <TouchableOpacity style={styles.secondaryDetailButton} activeOpacity={0.85} onPress={onViewFollowUps}>
              <Text style={styles.secondaryDetailButtonText} numberOfLines={1}>Ver seguimientos</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.aiDetailButton} activeOpacity={0.85}>
              <Sparkles size={13} color="#c78d1c" />
              <Text style={styles.aiDetailButtonText} numberOfLines={1}>Usar IA</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.primaryDetailButton} activeOpacity={0.85}>
              <Text style={styles.primaryDetailButtonText} numberOfLines={1}>Registrar seguimiento</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Pressable>
...
function NextActionDatePicker({
  ...
  const [pickerMode, setPickerMode] = useState<NextActionPickerMode | null>(null)
