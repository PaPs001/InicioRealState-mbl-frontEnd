...
import {
  ...
  getRegisterAccessErrorMessage,
  getRegisterExistingAccountConflict,
  getRegisterAccessParams,
  getRegisterAccessValidationErrors,
  getRegisterClientTypeFromRoute,
  isRegisterAccessFormValid,
  type RegisterAccessField,
} from '@/lib/services/register-user-access'
import { requestRegisterVerificationCode } from '@/lib/services/register-user-verification'
...
export default function RegisterOwnerAccessScreen() {
  ...
  const handleContinue = async () => {
    if (!isRegisterAccessFormValid(formData) || isSubmitting) {
      setShowError(true)
      if (!isSubmitting) {
        Alert.alert('Revisa tus datos', getRegisterAccessValidationErrors(formData).join('\n'))
      }
      return
    }

    const nextParams = getRegisterAccessParams(formData, getRegisterClientTypeFromRoute(pathname))
    setIsSubmitting(true)
    try {
      await requestRegisterVerificationCode(nextParams)
      router.push({
        pathname: '/regInquilino/verify',
        params: nextParams,
      })
    } catch (error) {
      const conflictField = getRegisterExistingAccountConflict(error)

      if (conflictField) {
        Alert.alert(
          conflictField === 'email' ? 'Este correo ya esta registrado' : 'Este telefono ya esta registrado',
          conflictField === 'email'
            ? 'Ya existe una cuenta con este correo. Puedes iniciar sesion o usar otro correo para registrarte.'
            : 'Ya existe una cuenta con este telefono. Puedes iniciar sesion o usar otro telefono para registrarte.',
          [
            {
              text: conflictField === 'email' ? 'Usar otro correo' : 'Usar otro telefono',
              style: 'cancel',
              onPress: () => updateField(conflictField, ''),
            },
            {
              text: 'Iniciar sesion',
              onPress: () => router.replace('/login/login' as never),
            },
          ],
        )
        return
      ...
      Alert.alert('No se pudo enviar el codigo', getRegisterAccessErrorMessage(error))
    } finally {
          ...
          <View style={registerOwnerAccessStyles.main}>
            <View style={registerOwnerAccessStyles.topSection}>
              ...
              <View style={registerOwnerAccessStyles.header}>
                ...
                <Text style={registerOwnerAccessStyles.subtitle}>
                  Ingresa tu informacion para activar tu perfil de inquilino
                </Text>
              </View>
            ...
            <View style={registerOwnerAccessStyles.infoCard}>
              ...
              <Text style={registerOwnerAccessStyles.infoText}>
                Tu acceso sera exclusivo a tu perfil de inquilino.
              </Text>
            </View>
            ...
            <TouchableOpacity
              style={[
                ...
                (!isValid || isSubmitting) && registerOwnerAccessStyles.continueButtonDisabled,
              ]}
              onPress={handleContinue}
              activeOpacity={0.84}
              >
                <Text style={registerOwnerAccessStyles.continueButtonText}>{isSubmitting ? 'Enviando...' : 'Continuar'}</Text>
              <ArrowRight
