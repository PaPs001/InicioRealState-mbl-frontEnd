...
import {
  getOwnerProfileRegisterParams,
  isOwnerProfileSelectionComplete,
  ownerPrimaryInterestOptions,
  ownerPriorityOptions,
  ownerPropertyProfileOptions,
  type OwnerProfileOption,
  type OwnerProfileSelection,
} from '@/lib/services/register-owner-profile'
import { registerOwnerProfileStyles } from './register-owner-profile.styles'
...
export default function RegisterOwnerProfileScreen() {
  ...
  const [selection, setSelection] = useState<OwnerProfileSelection>({})
  const [showError, setShowError] = useState(false)

  const updateSelection = (field: keyof OwnerProfileSelection, value: string) => {
    setShowError(false)
  ...
  const handleContinue = () => {
    if (!isOwnerProfileSelectionComplete(selection)) {
      setShowError(true)
      return
    }

    router.push({
      pathname: '/register-owner-final' as never,
      params: getOwnerProfileRegisterParams(params, selection),
    })
  }

  const isComplete = isOwnerProfileSelectionComplete(selection)
        ...
        <View style={registerOwnerProfileStyles.main}>
          <View style={registerOwnerProfileStyles.header}>
            ...
            <View style={registerOwnerProfileStyles.titleBlock}>
              <Text style={registerOwnerProfileStyles.title}>Queremos conocer tu perfil</Text>
          ...
          <View style={registerOwnerProfileStyles.sections}>
            <OptionGrid
              ...
              selectedId={selection.propertyProfile}
              onSelect={(id) => updateSelection('propertyProfile', id)}
            />
            <OptionGrid
              title="Me interesa principalmente"
              options={ownerPrimaryInterestOptions}
              selectedId={selection.primaryInterest}
              onSelect={(id) => updateSelection('primaryInterest', id)}
            />
            <OptionGrid
              title="Mis prioridades"
              options={ownerPriorityOptions}
              selectedId={selection.priority}
              onSelect={(id) => updateSelection('priority', id)}
            />
          </View>
