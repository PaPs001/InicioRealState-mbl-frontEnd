...
import { loginNewStyles } from './login-new.styles'
...
export default function LoginNewScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const { user, token } = await signInWithCredentials(email, password)
      ...
      console.info('[auth][login-new][navigation]', {
        ...
        tenant: user.tenant,
        destination: user.systemRole === 'ADMIN' || user.systemRole === 'COORDINATOR' ? '/coordinator' : '/(tabs)',
      })
      router.replace(user.systemRole === 'ADMIN' || user.systemRole === 'COORDINATOR' ? '/coordinator' : '/(tabs)')
    } catch (error) {
