...
import {
  getInitialOwnerAccessFormData,
  getOwnerAccessRegisterParams,
  isOwnerAccessFormValid,
  type OwnerAccessField,
} from '@/lib/services/register-owner-access'
import { registerOwnerAccessStyles } from './register-owner-access.styles'

export default function RegisterOwnerAccessScreen() {
  const router = useRouter()
  const [formData, setFormData] = useState(getInitialOwnerAccessFormData)
  const [showError, setShowError] = useState(false)

  const updateField = (field: OwnerAccessField, value: string) => {
    setShowError(false)
    setFormData((current) => ({
      ...current,
      [field]: value,
  ...
  const handleContinue = () => {
    if (!isOwnerAccessFormValid(formData)) {
      setShowError(true)
      return
    }
    ...
    router.push({
      ...
      params: getOwnerAccessRegisterParams(formData),
    })
  }

  const isValid = isOwnerAccessFormValid(formData)
