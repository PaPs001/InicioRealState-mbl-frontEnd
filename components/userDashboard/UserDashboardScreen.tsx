...
import { Bell, CalendarDays, ChevronLeft, ChevronRight, Eye, Home, LogOut, Plus, Settings } from 'lucide-react-native'
...
import { createGoogleCalendarDate, disconnectGoogleCalendar, getBackendLeadRecords, getGoogleCalendarAuthUrl, getGoogleCalendarConnectionStatus, getGoogleCalendarDates, getGoogleCalendars, getSelectedGoogleCalendars, saveSelectedGoogleCalendars, syncGoogleCalendars, type CreateGoogleCalendarDatePayload, type GoogleCalendarOption, type SelectedGoogleCalendar } from '@/lib/api'
...
export function UserDashboardScreen({ area }: UserDashboardScreenProps) {
  ...
  const advisorName = currentUser?.name?.trim() || currentUser?.email?.split('@')[0] || areaConfig.fallbackName
  ...
  ] satisfies DashboardPriority[], [leadSummary, rentSummary.propertyCount])
  ...
  const handleLogout = () => Alert.alert('Cerrar sesion', 'Estas seguro que deseas cerrar sesion?', [
    ...
    { text: 'Cerrar sesion', style: 'destructive', onPress: async () => { setIsProfileMenuOpen(false); await logout(); router.replace('/login/login' as never) } },
  ])
      ...
      <SafeAreaView style={styles.safeArea} edges={['left', 'right', 'bottom']}>
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.sectionHeader}>
            <TouchableOpacity style={styles.notification} activeOpacity={0.85} onPress={() => setIsSettingsOpen(false)}>
              <ChevronLeft size={22} color="#315b41" />
            </TouchableOpacity>
            <View>
              <Text style={styles.sectionHeaderTitle}>Configuracion</Text>
              <Text style={styles.panelSubtitle}>Calendarios y conexion de Google</Text>
            </View>
          </View>
          ...
          <View style={styles.googleCalendarSettings}>
            ...
            {googleCalendars.length === 0 ? (
              ...
              <View style={styles.calendarList}>{googleCalendars.map(calendar => {
    ...
    <SafeAreaView style={styles.safeArea} edges={['left', 'right', 'bottom']}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.topRow}>
          ...
          <View style={styles.datePill}><Text style={styles.dateText}>{formatCurrentDashboardDate()}</Text></View>
        </View>
          ...
          {isProfileMenuOpen ? (
            <View style={styles.profileMenu}>
              ...
              >
                ...
                <Text style={styles.profileMenuButtonText}>Configuraciones</Text>
              </TouchableOpacity>
              <TouchableOpacity
        ...
        <View style={styles.heroCards}>
          <TouchableOpacity style={styles.availableCard} activeOpacity={0.85} onPress={() => router.push(`${areaConfig.basePath}/properties` as never)}>
        ...
        <View style={[styles.panel, styles.appointmentsPanel]}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity activeOpacity={0.85} onPress={() => setIsSettingsOpen(true)}>
          ...
          <View style={styles.appointmentActionsRow}>
            <TouchableOpacity style={styles.centerButton} activeOpacity={0.85} onPress={() => loadCalendarDates({ sync: true })} disabled={isCalendarLoading}>
              ...
              <Text style={styles.centerButtonText}>{isCalendarLoading ? 'Cargando...' : 'Recargar calendario'}</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.centerButton} activeOpacity={0.85} onPress={handleOpenAppointmentModal}>
        ...
        <View style={[styles.panel, styles.leadPanel]}>
          ...
          <View style={styles.metricGrid}>{leadSummary.leadMetrics.map(metric => 
            <LeadMetricCard key={metric.id} metric={metric} />)}
          </View>
          <Text style={styles.subTitle}>Vista rapida</Text>
          <View style={styles.funnelRow}>{leadSummary.leadFunnel.map(metric => 
