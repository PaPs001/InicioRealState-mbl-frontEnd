...
function isMeaningfulLeadName(value?: string | number | null) {
  ...
  return Boolean(
    normalizedValue &&
    normalizedValue.toLowerCase() !== EMPTY_LEAD_NAME.toLowerCase(),
  );
}

function getFirstMeaningfulValue(
...
function getBackendLeadSearchIntent(
  lead: BackendLead,
): PropertyLead["searchIntent"] {
  const operation = getFirstMeaningfulValue(
    lead.typeOfOperation,
    lead.operation,
  ).toLowerCase();
...
function getBackendAdvisorName(lead: BackendLead) {
  ...
  return (
    ...
    lead.agentName ||
    lead.agent ||
    (typeof lead.advisor === "string" ? lead.advisor : undefined) ||
    undefined
  );
}
...
export function mapBackendLeadToPropertyLead(lead: BackendLead): PropertyLead {
  ...
  const firstContactDate = normalizeDate(
    lead.dateFirstContact || lead.createdAt,
  );
  const advisorId = getBackendAdvisorId(lead);
  const campaign = getFirstMeaningfulValue(lead.campaign, lead.adsCampain);
  const budget = getFirstMeaningfulValue(
    lead.maximumBudget,
    lead.maximunBudget,
    lead.maxinumBudget,
    lead.minimumBudget,
...
export function mapBackendFollowUpToLeadFollowUp(
  followUp: BackendFollowUp,
): LeadFollowUp {
  const leadId =
    followUp.leadId || followUp.clientId || followUp.client || undefined;
  const notes =
  ...
  return {
    ...
    nextActionDate: followUp.nextContact
      ? normalizeDate(followUp.nextContact)
      : undefined,
  };
}
...
export async function createBackendLeadV2Record(
  ...
  const propertyOfInterestId =
    payload.propertyOfInterestId?.trim() || "manual-lead";
...
export async function getBackendLeadV2Statuses(
  ...
  const response = await coreApi<BackendLeadV2StatusesResponse>(
    "/leads-v2/statuses",
    {
      token: token ?? undefined,
    },
...
export async function setBackendLeadV2Status(
  ...
  const response = await coreApi<{ lead: BackendLead; statuses: string[] }>(
    `/leads-v2/${leadId}/status`,
    {
      method: "POST",
      token: token ?? undefined,
      body: payload,
...
export async function getBackendLeadV2Followings(
  ...
  const followings = await coreApi<BackendLeadV2Following[]>(
    `/leads-v2/${leadId}/followings`,
    {
      token: token ?? undefined,
    },
  );
...
export async function createBackendLeadV2Following(
  ...
  formData.append(
    "contactDate",
    payload.contactDate || new Date().toISOString(),
  );
  ...
  return mapBackendLeadV2Following(
    (await response.json()) as BackendLeadV2Following,
  );
}
...
function mapBackendLeadV2Following(
  ...
  return {
    ...
    createdAt: following.createdAt
      ? normalizeDate(following.createdAt)
      : undefined,
