...
export function CreateLeadFollowingModal({
  leadId,
  onClose,
  onCreated,
  visible,
  ...
  const pickImage = async () => {
    ...
    if (!permission.granted)
      return setError("Necesitamos permiso para escoger una imagen.");
  ...
  const submit = async () => {
    if (!authToken || !leadId)
      return setError("No hay sesion o lead valido para guardar.");
    if (!text.trim() && !selectedImage)
      return setError("Agrega texto o una imagen para guardar el seguimiento.");
    setIsSaving(true);
    setError(null);
    try {
      await createBackendLeadV2Following(
        leadId,
        {
          text,
  ...
  return (
    <AppModal
      ...
      subtitle="Guarda texto o una imagen en el seguimiento"
      animationType="slide"
      showCloseButton
      position="bottom"
      keyboardAvoiding
      ...
      scrollable
      footerStyle={styles.compactFooter}
      containerStyle={styles.followingModalContainer}
      footer={
        <TouchableOpacity
      ...
      >
        ...
        <View style={styles.imagePickerCopy}>
          <Text style={styles.imagePickerTitle}>
            {selectedImage ? "Imagen seleccionada" : "Agregar imagen opcional"}
          </Text>
